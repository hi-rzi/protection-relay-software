/*
 * Page-specific glue for the GSUT reference page — field lists + page quirks,
 * mirroring views/transformer_gsut.py's script body. All heavy math (relay
 * evaluation, fault current, CT-tap trig) is server-side via
 * /api/transformer/gsut/recompute; cheap arithmetic (mismatch %, bias/min-op
 * floors, boundary injection, sweep table) is client-side per the Phase 1 plan.
 */
(function () {
  "use strict";

  var RECOMPUTE_URL = "/api/transformer/gsut/recompute";
  var SETTINGS_SHEET_URL = "/api/transformer/gsut/settings-sheet.csv";
  var REPORT_PDF_URL = "/api/transformer/gsut/report.pdf";
  ...
  var SETTINGS_FIELDS = [
    ...
    "fault_mva_base", "z_pct", "x_over_r", "ct_withstand_a", "fault_kv_hv",
  ];

  var initialData = JSON.parse(document.getElementById("initial-data").textContent);
  var equipmentTag = "gsut";
  ...
  function currentSettings() {
    ...
    // (web/services/gsut.py's compute_fault_current reads these from `settings`,
  ...
  function applyPreset(presetName) {
    ...
    var fieldValues = {
      ...
      ct_withstand_a: data.ct_withstand_a, fault_kv_hv: data.fault_kv_hv,
    };
  ...
  function downloadSweepCsv() {
    ...
    window.Recompute.downloadBlob(blob, "87GT_Sweep_Test_Table_" + stamp + ".csv");
  }
  ...
  function downloadSettingsSheet() {
    window.Recompute.postForBlob(SETTINGS_SHEET_URL, buildRecomputePayload()).then(function (blob) {
      window.Recompute.downloadBlob(blob, "GSUT_Settings_Sheet.csv");
    });
  }
  ...
  function exportReport() {
    window.Recompute.postForBlob(REPORT_PDF_URL, Object.assign(buildRecomputePayload(), { selected_preset: state.selectedPreset })).then(function (blob) {
      window.Recompute.downloadBlob(blob, "GSUT_Differential_Protection_Report.pdf");
    });
  }

  function saveProfile() {
    var name = document.getElementById("profile-name").value || "GSUT Profile";
  ...
  function renderAfterRecompute(data) {
    ...
    document.getElementById("rated-current-info").innerHTML =
      "HV (525kV) Nominal Rated Current: <strong>" + w[0].i_rated_pri.toFixed(1) + " A</strong>  |  " +
      "LV (23kV) Nominal Rated Current: <strong>" + w[1].i_rated_pri.toFixed(1) + " A</strong>";
