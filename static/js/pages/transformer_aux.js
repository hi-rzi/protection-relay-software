/*
 * Page-specific glue for the Aux/UAT page — field lists + page quirks,
 * mirroring views/transformer_aux.py's script body. All heavy math (relay
 * evaluation, fault current, CT-tap trig) is server-side via
 * /api/transformer/aux/recompute; cheap arithmetic (mismatch %, bias/min-op
 * floors, boundary injection, sweep table) is client-side per the Phase 1
 * plan. Ported from transformer_gsut.js.
 */
(function () {
  "use strict";

  var RECOMPUTE_URL = "/api/transformer/aux/recompute";
  var SETTINGS_SHEET_URL = "/api/transformer/aux/settings-sheet.csv";
  var REPORT_PDF_URL = "/api/transformer/aux/report.pdf";
  ...
  function currentSettings() {
    ...
    // (web/services/aux.py's compute_fault_current reads these from `settings`,
  ...
  function downloadSweepCsv() {
    ...
    window.Recompute.downloadBlob(blob, "87AT_Sweep_Test_Table_" + stamp + ".csv");
  }
  ...
  function downloadSettingsSheet() {
    window.Recompute.postForBlob(SETTINGS_SHEET_URL, buildRecomputePayload()).then(function (blob) {
      window.Recompute.downloadBlob(blob, "UAT_Settings_Sheet.csv");
    });
  }
  ...
  function exportReport() {
    window.Recompute.postForBlob(REPORT_PDF_URL, Object.assign(buildRecomputePayload(), { selected_preset: state.selectedPreset })).then(function (blob) {
      window.Recompute.downloadBlob(blob, "UAT_Differential_Protection_Report.pdf");
    });
  }

  function saveProfile() {
    var name = document.getElementById("profile-name").value || "UAT Profile";
  ...
  function renderAfterRecompute(data) {
    ...
    document.getElementById("rated-current-info").innerHTML =
      ...
      "LV (13.8kV) Nominal Rated Current: <strong>" + w[1].i_rated_pri.toFixed(1) + " A</strong>";
