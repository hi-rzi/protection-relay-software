...
from web.presets.motor_fd_fan import PRESETS
from web.services import fd_fan as svc

bp = Blueprint("motor_fd_fan", __name__, url_prefix="")


@bp.route("/motor/fd-fan")
def page():
...
@bp.route("/api/motor/fd-fan/recompute", methods=["POST"])
def recompute():
...
@bp.route("/api/motor/fd-fan/settings-sheet.csv", methods=["POST"])
def settings_sheet_csv():
    ...
    return Response(
        ...
        headers={"Content-Disposition": "attachment; filename=FD_Fan_Settings_Sheet.csv"},
    )


@bp.route("/api/motor/fd-fan/report.pdf", methods=["POST"])
def report_pdf():
    payload = request.get_json(force=True, silent=True) or {}
    settings = payload.get("settings", {})
    unit_name = payload.get("selected_preset", "FD Fan")
    ...
    pdf_buf = generate_fan_motor_pdf_report(
        f"Forced Draft (FD) Fan - {unit_name}", relay, eval_result, gf_eval, unbal_eval,
        test_current, ground_current, unbalance_pct, approval=approval,
    )

    return Response(
        pdf_buf.getvalue(), mimetype="application/pdf",
        headers={"Content-Disposition": "attachment; filename=FD_Fan_Protection_Report.pdf"},
    )
