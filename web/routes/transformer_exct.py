...
from web.presets.transformer_exct import PRESETS, MR_CT_TAPS_600_5
from web.services import exct as svc

bp = Blueprint("transformer_exct", __name__, url_prefix="")

PHASES = svc.PHASES


@bp.route("/transformer/exct")
def page():
    ...
    initial_data = {
        ...
        "mr_ct_taps_600_5": MR_CT_TAPS_600_5,
    }

    return render_template(
        "transformer_exct.html",
        presets=PRESETS,
        default_preset_name=default_preset_name,
...
@bp.route("/api/transformer/exct/recompute", methods=["POST"])
def recompute():
...
@bp.route("/api/transformer/exct/settings-sheet.csv", methods=["POST"])
def settings_sheet_csv():
    ...
    return Response(
        ...
        headers={"Content-Disposition": "attachment; filename=EXCT_Settings_Sheet.csv"},
    )


@bp.route("/api/transformer/exct/report.pdf", methods=["POST"])
def report_pdf():
    payload = request.get_json(force=True, silent=True) or {}
    settings = payload.get("settings", {})
    convention = str(payload.get("convention", "IEEE")).upper()
    ct_polarity = str(payload.get("ct_polarity", "OPPOSITE")).upper()
    unit_name = payload.get("selected_preset", "EXCT")
    ...
    return Response(
        ...
        headers={"Content-Disposition": "attachment; filename=EXCT_Differential_Protection_Report.pdf"},
    )
