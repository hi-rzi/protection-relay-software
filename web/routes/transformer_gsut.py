...
from web.presets.transformer_gsut import PRESETS, MR_CT_TAPS_2000_5
from web.services import gsut as svc

bp = Blueprint("transformer_gsut", __name__, url_prefix="")

PHASES = svc.PHASES


@bp.route("/transformer/gsut")
def page():
...
@bp.route("/api/transformer/gsut/recompute", methods=["POST"])
def recompute():
...
@bp.route("/api/transformer/gsut/settings-sheet.csv", methods=["POST"])
def settings_sheet_csv():
    ...
    return Response(
        ...
        headers={"Content-Disposition": "attachment; filename=GSUT_Settings_Sheet.csv"},
    )


@bp.route("/api/transformer/gsut/report.pdf", methods=["POST"])
def report_pdf():
    payload = request.get_json(force=True, silent=True) or {}
    settings = payload.get("settings", {})
    convention = str(payload.get("convention", "IEEE")).upper()
    ct_polarity = str(payload.get("ct_polarity", "OPPOSITE")).upper()
    unit_name = payload.get("selected_preset", "GSUT")
    ...
    return Response(
        ...
        headers={"Content-Disposition": "attachment; filename=GSUT_Differential_Protection_Report.pdf"},
    )
