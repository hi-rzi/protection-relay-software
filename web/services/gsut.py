"""
GSUT (Generator Step-Up Transformer, 2-winding CAC1-10-M3 differential relay)
calculation glue for the Flask reference page. Ports the arithmetic from
views/transformer_gsut.py's script body into plain functions that take a
...
def compute_mismatch(settings):
    """Mirrors lines 193-234 of views/transformer_gsut.py — CT matching tap
...
def bias_curve(relay, max_x=None, n=400):
    """Theoretical CAL. line, per-unit — matches views/transformer_gsut.py's
    Live Simulation bias-curve x/y arrays (lines 493-499), pu only (the
    frontend scales to Amps via amps_base for the pu/Amps chart-units toggle)."""
    if max_x is None:
...
def default_phase_a_angles(relay, ct_polarity):
    ...
    views/transformer_gsut.py lines 424-446 / _on_polarity_change."""
    def_ang_hv_a = 0.0
...
def compute_fault_current(settings, kv_lv):
    ...
    z_pct = _f(settings, "z_pct", 10.0)
    x_over_r = _f(settings, "x_over_r", 20.0)
    fault_kv_hv = _f(settings, "fault_kv_hv", 525.0)
...
def fault_waveform(fault_calc, x_over_r, cycles=5, freq_hz=60.0, n=800):
    """Instantaneous decaying-DC-offset waveform — mirrors views/transformer_gsut.py
    lines 893-899."""
    w = 2 * math.pi * freq_hz
...
def fault_clearing_sim(relay, fault_calc, fault_side, fault_scenario, relay_operate_cycles, breaker_cycles):
    """Mirrors views/transformer_gsut.py lines 954-1053 — evaluates whether the
...
def boundary_injection(relay, r_val):
    """Boundary Injection Calculator — mirrors lines 576-582."""
...
def recompute(payload):
    ...
    posted settings JSON (see web/routes/transformer_gsut.py for shape)."""
    ...
    if "fault_mva_base" in settings or "z_pct" in settings:
        ...
        out["fault"] = fault
        out["fault"]["waveform"] = fault_waveform(fault["fault_calc"], _f(settings, "x_over_r", 20.0))

        fault_sim_req = payload.get("fault_sim")
        if fault_sim_req:
            out["fault"]["sim"] = fault_clearing_sim(
                relay, fault["fault_calc"], fault["fault_side"],
                fault_sim_req.get("fault_scenario", "Internal Fault (within 87GT zone)"),
