"""
Aux / UAT (Unit Auxiliary Transformer, 2-winding CAC1-10-M3 differential relay)
calculation glue for the Flask reference page. Ports the arithmetic from
views/transformer_aux.py's script body into plain functions that take a
...
Like GSUT (and unlike EXCT), the UAT settings doc's kv_hv is tap-adjusted
(22.425kV, Tap Position 3) rather than the transformer's actual HV nameplate
voltage (23kV) — so compute_fault_current here uses a separate fault_kv_hv
field for the HV fault-current base, matching views/transformer_aux.py lines
839-846 and web/services/gsut.py's same pattern.
"""
import math
    ...
    return [
        {
            ...
            "ct_connection": str(settings.get("ct_conn_hv", "WYE")).upper(),
        },
        {
            "name": "LV (13.8kV)",
            ...
            "ct_connection": str(settings.get("ct_conn_lv", "DELTA")).upper(),
        },
    ]
...
def compute_mismatch(settings):
    """Mirrors lines 204-247 of views/transformer_aux.py — CT matching tap
    ...
    ct_conn_hv = str(settings.get("ct_conn_hv", "WYE")).upper()
    ct_conn_lv = str(settings.get("ct_conn_lv", "DELTA")).upper()
...
def bias_curve(relay, max_x=None, n=400):
    """Theoretical CAL. line, per-unit — matches views/transformer_aux.py's
...
def default_phase_a_angles(relay, ct_polarity):
    ...
    views/transformer_aux.py lines 434-457 / _on_polarity_change."""
    def_ang_hv_a = 0.0
...
def compute_fault_current(settings, kv_lv):
    """Mirrors views/transformer_aux.py lines 857-863 — HV fault base is the
    dedicated fault_kv_hv field (nameplate voltage), NOT settings' own
    tap-adjusted kv_hv."""
    fault_side = str(settings.get("fault_side", "HV")).upper()
    fault_mva_base = _f(settings, "fault_mva_base")
    z_pct = _f(settings, "z_pct", 8.47)
    x_over_r = _f(settings, "x_over_r", 38.9)
    fault_kv_hv = _f(settings, "fault_kv_hv", 23.0)
...
def fault_waveform(fault_calc, x_over_r, cycles=5, freq_hz=60.0, n=800):
    """Instantaneous decaying-DC-offset waveform — mirrors views/transformer_aux.py
    lines 899-905."""
    w = 2 * math.pi * freq_hz
...
def fault_clearing_sim(relay, fault_calc, fault_side, fault_scenario, relay_operate_cycles, breaker_cycles):
    """Mirrors views/transformer_aux.py lines 960-1059 — evaluates whether the
...
def boundary_injection(relay, r_val):
    """Boundary Injection Calculator — mirrors lines 582-592."""
...
def recompute(payload):
    ...
    posted settings JSON (see web/routes/transformer_aux.py for shape)."""
    ...
    if "fault_mva_base" in settings or "z_pct" in settings:
        kv_lv = _f(settings, "kv_lv")
        fault = compute_fault_current(settings, kv_lv)
        out["fault"] = fault
        out["fault"]["waveform"] = fault_waveform(fault["fault_calc"], _f(settings, "x_over_r", 38.9))

        fault_sim_req = payload.get("fault_sim")
        if fault_sim_req:
            out["fault"]["sim"] = fault_clearing_sim(
                relay, fault["fault_calc"], fault["fault_side"],
                fault_sim_req.get("fault_scenario", "Internal Fault (within 87AT zone)"),
