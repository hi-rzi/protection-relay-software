...
from common.ui_helpers import slider_with_exact_input, MR_CT_TAPS_600_5, fault_term_info, sidebar_section_nav, equipment_switcher
...
equipment_switcher("views/transformer_exct.py")
...
selected, c, pinned = sidebar_section_nav(sections, key_prefix="exct", pin_first=True)
...
with c["Settings Calculator"]:
    ...
    with st.container(border=True):
        st.markdown("**Ratings and CT**")
        calc_mva = st.number_input("Transformer Rating (MVA)", min_value=0.1, value=float(mva), step=1.0, key="exct_calc_mva")
        calc_ct_sec = st.number_input("CT Secondary Rating (A)", min_value=1.0, value=float(ct_secondary_rating), step=1.0, key="exct_calc_ct_sec")
        ecalc1, ecalc2 = st.columns(2)
        with ecalc1:
            st.markdown("HV Winding")
            calc_kv_hv = st.number_input("HV Rated Voltage (kV)", min_value=0.1, value=float(kv_hv), step=1.0, key="exct_calc_kv_hv")
            calc_ct_hv = st.number_input("HV CT Ratio (Primary A)", min_value=1.0, value=float(ct_hv), step=1.0, key="exct_calc_ct_hv")
            calc_conn_hv = st.selectbox("HV CT Connection", ["DELTA", "WYE"], index=0 if ct_conn_hv.upper() == "DELTA" else 1, key="exct_calc_conn_hv")
            calc_tap_hv = st.number_input("HV Tap (as set)", min_value=0.1, value=float(tap_hv), step=0.01, key="exct_calc_tap_hv")
        with ecalc2:
            st.markdown("LV Winding")
            calc_kv_lv = st.number_input("LV Rated Voltage (kV)", min_value=0.1, value=float(kv_lv), step=1.0, key="exct_calc_kv_lv")
            calc_ct_lv = st.number_input("LV CT Ratio (Primary A)", min_value=1.0, value=float(ct_lv), step=1.0, key="exct_calc_ct_lv")
            calc_conn_lv = st.selectbox("LV CT Connection", ["DELTA", "WYE"], index=0 if ct_conn_lv.upper() == "DELTA" else 1, key="exct_calc_conn_lv")
            calc_tap_lv = st.number_input("LV Tap (as set)", min_value=0.1, value=float(tap_lv), step=0.01, key="exct_calc_tap_lv")
        ...
        calc_mismatch_exct = mismatch_ratio_pct([calc_i_relay_hv, calc_i_relay_lv])
        st.metric("Mismatch at entered taps", f"{calc_mismatch_exct:.2f}%" if calc_mismatch_exct is not None else "—")

    calc_suggestion = suggest_bias_settings(calc_mismatch_exct or 0.0, num_windings=2)
...
with c["Simulate & Test"]:
    ...
    tp_source = st.radio(
        ...
        key="exct_tp_source", help=TEST_POINT_SOURCE_HELP,
    )
...
with c["Fault Current Analysis"]:
    ...
    with fmc1:
        ...
        asym_basis = st.radio(
            ...
            help="Actual X/R: computes how much the DC offset has decayed by 0.5 cycle, using "
                 "this transformer's real X/R above (this is what the settings doc's own worked "
                 "example does). Worst-case: skips that decay and uses the flat sqrt(3)=1.73 "
                 "ceiling instead (X/R -> infinity). Worst-case is always >= the Actual X/R result."
        )
    ...
    fault_scenario = st.radio(
        "Fault Scenario",
        ["Internal Fault (within 87ET zone)", "External Through-Fault"],
        ...
        help=(
            ...
            "healthy 87ET should NOT operate — it's outside this differential zone's job to "
            "clear it."
        ),
    )
    ...
    if run_sim:
        if sim_eval["is_trip"]:
            ...
            if fault_scenario.startswith("External"):
                sim_caption_ph.info("🛡️ Relay checks the fault — operate current stays at zero. 87ET correctly stays SECURE; clearing this through-fault is outside its zone.")
            else:
...
with c["Settings Summary & Approval"]:
    ...
    st.session_state.setdefault("exct_source_document", "Transformer Diff Setting - EXCT.pdf")
    st.session_state.setdefault("exct_revision", "Rev. 0")
    st.session_state.setdefault("exct_prepared_by", "")
    st.session_state.setdefault("exct_reviewed_by", "")
    st.session_state.setdefault("exct_approval_status", "Draft — engineering review required")
    st.session_state.setdefault("exct_review_note", "")

    source_document = st.text_input("Source document", key="exct_source_document")
    col_doc_1, col_doc_2 = st.columns(2)
    with col_doc_1:
        revision = st.text_input("Document / settings revision", key="exct_revision")
        prepared_by = st.text_input("Prepared by", key="exct_prepared_by")
    with col_doc_2:
        reviewed_by = st.text_input("Reviewed by", key="exct_reviewed_by")
        approval_status = st.selectbox(
            "Review status",
            ["Draft — engineering review required", "Reviewed — pending approval", "Approved for issue"],
            key="exct_approval_status",
        )
    review_note = st.text_area("Review note / change description", key="exct_review_note")
    ...
    pdf_bytes = generate_transformer_pdf_report(
        selected_preset, relay, evals, phases, winding_currents=winding_currents,
        settings_sheets=[("CAC1-10-M3", _sheet_rows)],
    )
    st.download_button(
