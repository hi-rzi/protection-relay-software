...
4. **Work through "Analysis & Tools" in order** — the tabs are always in this sequence:
   ...
   - **Curve & Test Points** — the relay's own characteristic curve (a time-current curve for the motor pages' overcurrent elements, a bias/restraint curve for the generator and transformer differential relays), plus logging real test results against it. On the motor pages this is also where the fault-current tools live (see below) — motors don't have a separate impedance-based fault study, so there's no standalone Fault Current Analysis tab for them.
   - **Fault Current Analysis** *(generator and transformer pages)* — checks the CTs against a real fault current: a CT saturation check, the actual asymmetrical current waveform, and a step-by-step fault-clearing simulation. The motor pages have a lighter version of the last two (CT saturation check and fault-clearing simulation, no waveform — there's no real X/R data for a motor short-circuit contribution) inside Curve & Test Points instead.
