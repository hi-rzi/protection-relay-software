...
from common.ui_helpers import slider_with_exact_input, MR_CT_TAPS_3000_5, fault_term_info, card_section_nav, equipment_switcher
...
equipment_switcher("views/transformer_aux.py")
...
selected, c, pinned = card_section_nav(sections, key_prefix="aux", pin_first=True)
...
with c["Settings Calculator"]:
    ...
    with st.container(border=True):
        st.markdown("**Ratings and CT**")
        calc_mva = st.number_input("Transformer Rating (MVA)", min_value=0.1, value=float(mva), step=1.0, key="aux_calc_mva")
        calc_ct_sec = st.number_input("CT Secondary Rating (A)", min_value=1.0, value=float(ct_secondary_rating), step=1.0, key="aux_calc_ct_sec")
        ecalc1, ecalc2 = st.columns(2)
        with ecalc1:
            st.markdown("HV Winding")
            calc_kv_hv = st.number_input("HV Rated Voltage (kV)", min_value=0.1, value=float(kv_hv), step=1.0, key="aux_calc_kv_hv")
            calc_ct_hv = st.number_input("HV CT Ratio (Primary A)", min_value=1.0, value=float(ct_hv), step=1.0, key="aux_calc_ct_hv")
            calc_conn_hv = st.selectbox("HV CT Connection", ["DELTA", "WYE"], index=0 if ct_conn_hv.upper() == "DELTA" else 1, key="aux_calc_conn_hv")
            calc_tap_hv = st.number_input("HV Tap (as set)", min_value=0.1, value=float(tap_hv), step=0.01, key="aux_calc_tap_hv")
        with ecalc2:
            st.markdown("LV Winding")
            calc_kv_lv = st.number_input("LV Rated Voltage (kV)", min_value=0.1, value=float(kv_lv), step=1.0, key="aux_calc_kv_lv")
            calc_ct_lv = st.number_input("LV CT Ratio (Primary A)", min_value=1.0, value=float(ct_lv), step=1.0, key="aux_calc_ct_lv")
            calc_conn_lv = st.selectbox("LV CT Connection", ["DELTA", "WYE"], index=0 if ct_conn_lv.upper() == "DELTA" else 1, key="aux_calc_conn_lv")
            calc_tap_lv = st.number_input("LV Tap (as set)", min_value=0.1, value=float(tap_lv), step=0.01, key="aux_calc_tap_lv")
        ...
        calc_mismatch_aux = mismatch_ratio_pct([calc_i_relay_hv, calc_i_relay_lv])
        st.metric("Mismatch at entered taps", f"{calc_mismatch_aux:.2f}%" if calc_mismatch_aux is not None else "—")

    calc_suggestion = suggest_bias_settings(calc_mismatch_aux or 0.0, num_windings=2)
