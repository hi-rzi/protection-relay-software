...
from common.ui_helpers import slider_with_exact_input, MR_CT_TAPS_2000_5, fault_term_info, card_section_nav, equipment_switcher
...
restore_profile_uploader(st.sidebar, "gsut", f"{selected_preset}__", "gsut")
...
selected, c, pinned = card_section_nav(sections, key_prefix="gsut", pin_first=True)
...
with c["Current Settings"]:
    ...
    export_profile_button(
        st, "gsut", f"{selected_preset}__",
        default_name="GSUT Profile", button_key="gsut",
    )
...
with c["Simulate & Test"]:
    ...
    tp_source = st.radio(
        ...
        key="gsut_tp_source", help=TEST_POINT_SOURCE_HELP,
    )
    ...
    comm_chart_units = st.radio("Chart units", ["Per-Unit (pu)", "Secondary Amps (A)"], horizontal=True, key="gsut_test_chart_units")
